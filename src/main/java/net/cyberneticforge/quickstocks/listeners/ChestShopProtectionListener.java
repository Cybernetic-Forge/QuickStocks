...
public class ChestShopProtectionListener implements Listener {
    ...
    public void onProtectionCheck(ProtectionCheckEvent event) {
        ...
        // Get the sign and check if it's a company shop
        if (!(event.getSign().getState() instanceof Sign)) {
            return;
        }
        ...
        if (chestShopHook.canManageShop(ownerName.trim(), event.getPlayer())) {
            event.allow();
            logger.fine("Granted shop access to " + event.getPlayer().getName() + 
                       " for company '" + ownerName + "'");
        }
    }
}
